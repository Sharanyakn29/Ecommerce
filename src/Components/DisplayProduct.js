...
export default function DisplayProduct() {
  ...
  const {addToCart,handleSizeSelect,selectedSizes} = useContext(shopContext)
      ...
      <div className='col-lg-6 col-12 px-4 px-lg-0'>
        ...
        <div className='sizes d-flex text-center'>
            {product.sizes.map((size)=>(
              <button key={size} className='size m-2 p-lg-1' 
              style={{backgroundColor: selectedSizes[product.id]===size?"#323030":"#d9d6d6",
                      color:selectedSizes[product.id]===size?"white":"black" }} 
                      onClick={() => handleSizeSelect(product.id,size)} >{size}</button>
            ))
            }
        </div>
        <button onClick={() => addToCart(product)} className='btn btn-danger my-3'>Add to cart</button>
